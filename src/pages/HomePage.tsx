...
export const HomePage: React.FC = () => {
  ...
  const handleSearch = async (query: string) => {
    ...
    try {
      ...
      // Don't automatically set queue on search - let user select a track
    } catch (error) {
  ...
  const handleTrackSelect = (index: number) => {
    ...
    if (currentTracks.length > 0 && index >= 0 && index < currentTracks.length) {
      const selectedTrack = currentTracks[index];
      if (selectedTrack) {
        // Set queue and immediately play the selected track
  ...
  const handleFastForward = () => {
    const forwardTime = Math.min(playerState.duration, playerState.currentTime + 10);
    seekTo(forwardTime);
  };
    ...
    <div className="min-h-screen bg-[#2B2D42]">
      ...
      <div className="container mx-auto px-4 py-4 md:py-8 pb-32 relative z-10">
        ...
        {currentView === 'search' && !hasSearched && (
          <div className="text-center py-16">
            ...
            <h2 className="text-xl font-semibold text-white mb-2">Welcome to Geethika Music Player</h2>
      ...
      <footer className="relative z-10 mt-16 py-8 border-t border-white/10">
        <div className="container mx-auto px-4 md:px-6 text-center">
          <div className="bg-gradient-to-r from-[#FF3CAC]/10 to-[#784BA0]/10 backdrop-blur-sm border border-white/10 rounded-xl md:rounded-2xl p-4 md:p-6 max-w-sm md:max-w-md mx-auto">
            <div className="flex items-center justify-center space-x-1 md:space-x-2 mb-2 md:mb-3">
              <span className="text-2xl">🎵</span>
              <span className="text-2xl">💛</span>
            </div>
            <p className="text-white font-medium text-sm md:text-lg mb-1 md:mb-2 leading-relaxed">
              A Raksha Bandhan gift from your brother,
            </p>
            <p className="text-[#FF3CAC] font-semibold text-base md:text-xl">
              with love & music.
            </p>
            <div className="mt-3 md:mt-4 flex items-center justify-center space-x-1">
